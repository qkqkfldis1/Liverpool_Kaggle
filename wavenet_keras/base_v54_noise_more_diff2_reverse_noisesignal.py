...
def read_data():
    ...
    STD = 0.01
    old_data = noised_train['signal']
    new_data = old_data + np.random.normal(0, STD, size=len(noised_train))
    noised_train['signal'] = new_data
    
    train = pd.concat([train, noised_train], axis=0).reset_index(drop=True)
...
def run_cv_model_by_batch(train, test, splits, batch_col, feats, sample_submission, nn_epochs, nn_batch_size):
    ...
    for n_fold, (tr_idx, val_idx, val_orig_idx) in enumerate(new_splits[0:], start=0):
        ...
        model.save_weights(f'models/model_fold_{n_fold}_v54_noise_more_diff2_reverse_noisesignal.ckpt')
    ...
    sample_submission.to_csv('submission_wavenet_base_v54_noise_more_diff2_reverse_noisesignal.csv', index=False, float_format='%.4f')
    
    np.save('preds_base_v54_noise_more_diff2_reverse_noisesignal.npy', preds_)
    np.save('oof_base_v54_noise_more_diff2_reverse_noisesignal.npy', oof_)
