...
def Classifier(shape_):
    ...
    x = cbr(inp, 64, 1, 1, 1)
    x = BatchNormalization()(x)
    x = wave_block(x, 16, 1, 12)
    x = BatchNormalization()(x)
    x = wave_block(x, 32, 1, 8)
    x = BatchNormalization()(x)
    x = wave_block(x, 64, 1, 4)
    x = BatchNormalization()(x)
    x = wave_block(x, 128, 1, 1)
    x = cbr(x, 32, 1, 1, 1)
    x = BatchNormalization()(x)
    x = Dropout(0.2)(x)
...
def run_cv_model_by_batch(train, test, splits, batch_col, feats, sample_submission, nn_epochs, nn_batch_size):
    ...
    sample_submission.to_csv('submission_wavenet_base_v54_noise_more_diff2_reverse_smallkernel.csv', index=False, float_format='%.4f')
    
    np.save('preds_base_v54_noise_more_diff2_reverse_smallkernel.npy', preds_)
    np.save('oof_base_v54_noise_more_diff2_reverse_smallkernel.npy', oof_)
