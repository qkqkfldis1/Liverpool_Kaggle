...
def run_cv_model_by_batch(train, test, splits, batch_col, feats, sample_submission, nn_epochs, nn_batch_size):
    ...
    sample_submission.to_csv('submission_wavenet_base_v54_noise_more_reverse.csv', index=False, float_format='%.4f')
    
    np.save('preds_base_v54_noise_more_reverse.npy', preds_)
    np.save('oof_base_v54_noise_more_reverse.npy', oof_)
