...
# read data
def read_data():
    train = pd.read_csv('/home/lyh/liverpool/train_clean_kalman.csv', dtype={'time': np.float32, 'signal': np.float32, 'open_channels':np.int32})
    test  = pd.read_csv('/home/lyh/liverpool/test_clean_kalman.csv', dtype={'time': np.float32, 'signal': np.float32})
...
def run_cv_model_by_batch(train, test, splits, batch_col, feats, sample_submission, nn_epochs, nn_batch_size):
    ...
    sample_submission.to_csv('submission_wavenet_base_v54_noise_more_diff2_reverse_kalman.csv', index=False, float_format='%.4f')
    
    np.save('preds_base_v54_noise_more_diff2_reverse_kalman.npy', preds_)
    np.save('oof_base_v54_noise_more_diff2_reverse_kalman.npy', oof_)
