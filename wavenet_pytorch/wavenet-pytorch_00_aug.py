...
class IronDataset(Dataset):
    def __init__(self, data, labels, training=True, transform=None, seq_len=5000, flip=0.5, noise_level=0, class_split=0.0, mode='train'):
        self.data = data
        self.labels = labels
...
for index, (train_index, val_index, _) in enumerate(new_splits[0:], start=0):
    ...
    train_dataset = IronDataset(train[train_index], train_tr[train_index], seq_len=GROUP_BATCH_SIZE, flip=flip, noise_level=noise, mode='train')
    train_dataloader = DataLoader(train_dataset, NNBATCHSIZE, shuffle=True, num_workers=0)

    valid_dataset = IronDataset(train[val_index], train_tr[val_index], seq_len=GROUP_BATCH_SIZE, flip=False)
    valid_dataloader = DataLoader(valid_dataset, NNBATCHSIZE, shuffle=False, num_workers=0)
    ...
    early_stopping = EarlyStopping(patience=40, is_maximize=True,
                                   checkpoint_path=os.path.join(outdir, "gru_clean_checkpoint_fold_{}_iter_{}_00_aug.pt".format(index,
                                                                                                             it)))
...
test_pred_frame.to_csv("./gru_preds_label_smooth_00_aug.csv", index=False)
np.save('test_oofs_label_smooth_00_aug.npy', test_preds_all)
