...
for index, (train_index, val_index, _) in enumerate(new_splits[0:], start=0):
    ...
    early_stopping = EarlyStopping(patience=40, is_maximize=True,
                                   checkpoint_path=os.path.join(outdir, "gru_clean_checkpoint_fold_{}_iter_{}_01_aug.pt".format(index,
                                                                                                             it)))
...
test_pred_frame.to_csv("./gru_preds_label_smooth_01_aug.csv", index=False)
np.save('test_oofs_label_smooth_01_aug.npy', test_preds_all)
